...
if iter == 10
    ...
    input.x0 = zeros(settings.nx,1);
    input.x0(6) = settings.speed_min; %vx
    input.u0 = zeros(settings.nu,1);

    input.x=repmat(input.x0,1,settings.N+1);           % (nx by N+1)
    input.u=repmat(input.u0,1,settings.N);           % (nu by N)
...
elseif mem.iter == 10 % 10th iteration settings update
...
curr_solver_problems = 0;
while 1
    try
        [output, mem] = mpc_nmpcsolver(input, settings, mem, opt);
        if any(sum(isnan(output.x))) || any(sum(isnan(output.u)))
            error('NaN in the NMPC solution.')
        end
        break
        ...
        curr_solver_problems = curr_solver_problems+1;
        if curr_solver_problems >= 5

            warning('failed NMPC 5 times in a row.');
            reset = 1;
            stop = 1;
            ctrls = [zeros(settings.nu,1);0;stop;reset;0;0;0;0];
            return
            ...
            input.u0 = zeros(settings.nu,1);
            x = repmat(input.x0,1,settings.N+1);  % initialize all shooting points with the same initial state
            u = repmat(input.u0,1,settings.N);    % initialize all controls with the same initial control
            z = repmat(input.z0,1,settings.N);    % initialize all algebraic state with the same initial condition
            input.x=x;           % states and controls of the first N stages (nx by N+1 matrix)
            input.u=u;           % (nu by N vector)
            input.z=z;           % (nz by N)
            input.lambda    = zeros(settings.nx,settings.N+1);   % langrangian multiplier w.r.t. equality constraints
            input.mu        = zeros(settings.N*settings.nc+settings.ncN,1);   % langrangian multipliers w.r.t. general inequality constraints
            input.mu_u      = zeros(settings.N*settings.nu,1);   % langrangian multipliers w.r.t. input bounds
            input.mu_x      = zeros(settings.N*settings.nbx,1);  % langrangian multipliers w.r.t. state bounds
        end
    end
end
